...
angular.module('auth').factory('authenticationService', ['$injector',
    ...
    var requestShareSlateService = $injector.get('requestShareSlateService');
    var service = {};
    ...
    /**
     * The unique identifier of the local storage key which stores the result
     * of the last share_slate authentication attempt.
     *
     * @type String
     */
    var SS_AUTH_STORAGE_KEY = 'SHARESLATE_AUTH';
    ...
    service.ssAuthenticate = function ssAuthenticate(parameters) {

        // Attempt authentication
        return requestShareSlateService({
            method: 'POST',
            url: 'apis/auth_token.php',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
            data: $.param(parameters)
            ...
            // If authentication succeeds, handle received auth data
            .then(function ssAuthenticationSuccessful(data) {
                if(data.status === 'success'){
                    return data;
                }
                console.log('data', data);
                service.setSSToken(null);
                return null;
            })

            // If authentication fails, propogate failure to returned promise
            ['catch'](requestShareSlateService.createErrorCallback(function ssAuthenticationFailed(error) {
                // TODO
                //$rootScope.$broadcast('guacInvalidSSToken', parameters);
        }));
    ...
    service.updateCurrentToken = function updateCurrentToken(parameters) {
        ...
        var ss_token = service.getSSToken();
        if(parameters.ss_token && ss_token !== parameters.ss_token){
            service.setSSToken(parameters.ss_token);
            ss_token = parameters.ss_token;
        } else if(!parameters.ss_token && !ss_token){
            // TODO
            //$rootScope.$broadcast('guacInvalidSSToken', parameters);
        }

        service.ssAuthenticate({ auth_token: ss_token });
    ...
    service.getSSAuthentication = function getSSAuthentication() {
        if(cachedSSResult){
            return cachedSSResult;
        }
        return (cachedSSResult = localStorageService.getItem(SS_AUTH_STORAGE_KEY));
    }

    service.getSSToken = function getSSToken(){
        var result = service.getSSAuthentication();
        if(result){
            return result.token;
        }
        return null;
    ...
    service.setSSToken = function setSSToken(token) {

        // Clear the currently-stored result if the last attempt failed
        if (!token) {
            cachedSSResult = null;
            localStorageService.removeItem(SS_AUTH_STORAGE_KEY);
        }
        ...
        // Otherwise store the authentication attempt directly
            ...
            // Always store in cache
            cachedSSResult = token;
            var ss_user = service.getClaimFromSSToken(token);
            localStorageService.setItem(SS_AUTH_STORAGE_KEY, {token, ss_user});
    ...
    service.getClaimFromSSToken = function getClaimFromSSToken(token) {
        var user = {};
    ...
    service.getSSUser = function getDataSource() {

        // Return data source, if available
        var result = service.getSSAuthentication();
        if(result){
            return result.ss_user;
        }
